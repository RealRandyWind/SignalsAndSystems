...
void fft2D(int direction, int width, int height, double **re, double **im) {
  ...
  /* allocate temp colmn buffers */
  cre = (double *) malloc(MAX(width, height) * sizeof(double));
  cim = (double *) malloc(MAX(width, height) * sizeof(double));
   if(!cre || !cim) {
    exit(EXIT_FAILURE);
  }

  /* real to complex rows */
  for (r=0; r<height; ++r) {
    fft1D(direction, width, re[r], im[r]);
  }
  
  /* complex to complex colmns */  
  for (c=0; c<width; ++c) {
    for (r=0; r<height; ++r) {
      cre[r] = re[r][c];
      cim[r] = im[r][c];
    }
    fft1D(direction, height, cre, cim);
    for (r=0; r<height; ++r) {
      re[r][c] = cre[r];
      im[r][c] = cim[r];
    }
